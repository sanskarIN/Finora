...
public sealed class OnboardingViewModel : ViewModelBase
{
    ...
    public OnboardingViewModel(IFinanceStore store, IAppSettingsService settings)
    {
        _store = store;
        _settings = settings;
    ...
    public int MonthStart { get => _monthStart; set => SetProperty(ref _monthStart, Math.Clamp(value, 1, 28)); }
    public string OpeningBalance { get => _openingBalance; set => SetProperty(ref _openingBalance, value); }
    public bool SampleData { get => _sampleData; set => SetProperty(ref _sampleData, value); }
    ...
    private Task FinishAsync() => RunAsync(async () =>
    {
        ...
        if (accounts.Count == 0 && opening != 0)
            await _store.SaveAccountAsync(new Account
            {
                Name = "Opening balance",
                Type = AccountType.Bank,
                Currency = currency,
                OpeningBalanceMinor = Money.FromMajorUnits(opening, currency).MinorUnits
            });

        if (SampleData && (await _store.GetAccountsAsync()).Count == 0)
        {
            var id = await _store.SaveAccountAsync(new Account
            {
                Name = "Sample wallet",
                Type = AccountType.DigitalWallet,
                Currency = currency,
                OpeningBalanceMinor = Money.FromMajorUnits(10_000m, currency).MinorUnits
            });
            var category = (await _store.GetCategoriesAsync()).FirstOrDefault(item => item.Name == "Food");
            await _store.SaveTransactionAsync(TransactionFactory.Create(
                TransactionType.Expense,
                Money.FromMajorUnits(450m, currency).MinorUnits,
                currency,
                id,
                DateTimeOffset.UtcNow,
                category?.Id,
                "Sample merchant",
                "Synthetic sample data can be removed from Settings."));
        }
