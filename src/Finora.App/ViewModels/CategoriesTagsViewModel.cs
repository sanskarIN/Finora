...
public sealed class CategoriesTagsViewModel : ViewModelBase
{
    ...
    private Task CreateCategoryAsync() => RunAsync(async () => { var result = await _service.SaveCategoryAsync(null, CategoryName, CategoryIcon, ParentCategory?.Id); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("CategoriesCreated"); CategoryName = string.Empty; CategoryIcon = "tag"; ParentCategory = null; await LoadCoreAsync(); });
    private Task UpdateCategoryAsync() => RunAsync(async () => { if (SelectedCategory is null) throw new InvalidOperationException(L("CategoriesChooseUpdate")); var result = await _service.SaveCategoryAsync(SelectedCategory.Id, CategoryName, CategoryIcon, ParentCategory?.Id); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("CategoriesUpdated"); await LoadCoreAsync(); });
    private Task ArchiveCategoryAsync() => RunAsync(async () => { if (SelectedCategory is null) throw new InvalidOperationException(L("CategoriesChooseArchive")); var replacement = MergeTarget is { IsArchived: false } ? MergeTarget.Id : (Guid?)null; var result = await _service.ArchiveCategoryAsync(SelectedCategory.Id, replacement); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("CategoriesArchived"); await LoadCoreAsync(); });
    private Task RestoreCategoryAsync() => RunAsync(async () => { if (SelectedCategory is null) throw new InvalidOperationException(L("CategoriesChooseRestore")); var result = await _service.RestoreCategoryAsync(SelectedCategory.Id); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("CategoriesRestored"); await LoadCoreAsync(); });
    private Task MoveCategoryAsync(int direction) => RunAsync(async () => { if (SelectedCategory is null) throw new InvalidOperationException(L("CategoriesChooseMove")); var result = await _service.MoveCategoryAsync(SelectedCategory.Id, direction); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = direction < 0 ? L("CategoriesMovedUp") : L("CategoriesMovedDown"); await LoadCoreAsync(); });
    private Task MergeCategoryAsync() => RunAsync(async () => { if (SelectedCategory is null || MergeTarget is null) throw new InvalidOperationException(L("CategoriesChooseMerge")); var result = await _service.MergeCategoryAsync(SelectedCategory.Id, MergeTarget.Id); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("CategoriesMerged"); await LoadCoreAsync(); });
    private Task CreateTagAsync() => RunAsync(async () => { var result = await _service.SaveTagAsync(null, TagName, TagColor); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("TagsCreated"); TagName = string.Empty; TagColor = string.Empty; await LoadCoreAsync(); });
    private Task UpdateTagAsync() => RunAsync(async () => { if (SelectedTag is null) throw new InvalidOperationException(L("TagsChooseUpdate")); var result = await _service.SaveTagAsync(SelectedTag.Id, TagName, TagColor); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("TagsUpdated"); await LoadCoreAsync(); });
    private Task ArchiveTagAsync() => RunAsync(async () => { if (SelectedTag is null) throw new InvalidOperationException(L("TagsChooseArchive")); var result = await _service.ArchiveTagAsync(SelectedTag.Id); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("TagsArchived"); await LoadCoreAsync(); });
    private Task RestoreTagAsync() => RunAsync(async () => { if (SelectedTag is null) throw new InvalidOperationException(L("TagsChooseRestore")); var result = await _service.RestoreTagAsync(SelectedTag.Id); if (!result.IsSuccess) throw new InvalidOperationException(result.Error); Status = L("TagsRestored"); await LoadCoreAsync(); });
