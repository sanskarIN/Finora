...
public sealed class BackupAttachmentPathTamperTests
{
    [Fact]
    public async Task AuthenticatedReceiptPathEscape_IsRejectedWithoutMutation()
    {
        ...
        var corrupted = BackupTestCipher.RewriteJson(valid, BackupAttachmentFixture.Password, root =>
        {
            ...
            attachment["relativePath"] = "attachments/../../escaped-receipt.bin";
        });
        ...
        Assert.False(File.Exists(Path.Combine(Path.GetDirectoryName(fixture.Root)!, "escaped-receipt.bin")));
    }
}
